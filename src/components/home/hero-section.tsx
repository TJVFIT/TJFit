...
function HeroCommandPanel({ reduce, copy }: { reduce: boolean; copy: HomeLuxuryCopy["hero"] }) {
  const icons = [Dumbbell, Utensils, Gauge];
  ...
  return (
    <div className="tj-hero-command-panel relative mx-auto w-full max-w-[23.75rem] overflow-hidden rounded-[22px] border border-white/[0.08] bg-[rgba(13,15,18,0.58)] p-3.5 shadow-[0_30px_90px_rgba(0,0,0,0.34),inset_0_1px_0_rgba(255,255,255,0.07)] backdrop-blur-xl">
      ...
      <div className="relative flex items-center justify-between border-b border-white/[0.07] pb-4">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-[#71717A]">{copy.commandToday}</p>
        ...
        {copy.commandRows.map((row, index) => {
          ...
          >
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[12px] border border-white/[0.08] bg-[#0A0A0B]">
              <Icon className="h-[18px] w-[18px] text-[#67E8F9]" strokeWidth={1.6} />
            </span>
      ...
      <div className="relative mt-4 grid grid-cols-[1fr_auto] items-end gap-4 rounded-[18px] border border-white/[0.06] bg-[#08080A] p-4">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-[#71717A]">{copy.commandConsistency}</p>
