...
export function MembershipPricing({ locale }: { locale: Locale }) {
  ...
  return (
    <section className="mt-8">
      <div className="rounded-2xl border border-[#1E2028] bg-[#111215] p-6 sm:p-8">
        ...
        <div className="mt-6 flex items-center gap-2">
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm ${mode === "monthly" ? "bg-[#22D3EE] text-[#09090B]" : "border border-[#1E2028] text-[#A1A1AA]"}`}
          >
            {copy.monthly}
          </button>
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm ${mode === "annual" ? "bg-[#22D3EE] text-[#09090B]" : "border border-[#1E2028] text-[#A1A1AA]"}`}
          >
            {copy.annual}
          </button>
          {mode === "annual" ? (
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        ...
        <article className="rounded-2xl border border-cyan-400/35 bg-[#111215] p-6 shadow-[0_0_30px_rgba(34,211,238,0.12)]">
          <p className="inline-flex rounded-full border border-cyan-400/30 bg-cyan-400/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-cyan-300">
            {copy.cards.pro.badge}
          </p>
          <h3 className="mt-3 text-xl font-bold text-white">{copy.cards.pro.name}</h3>
          <p className="mt-3 text-3xl font-extrabold text-white transition-all duration-300">
            €{price.pro} <span className="text-sm font-medium text-[#A1A1AA]">{mode === "monthly" ? copy.perMonthSuffix : copy.perYearSuffix}</span>
          </p>
        ...
        <article className="rounded-2xl border border-violet-400/35 bg-[#111215] p-6 shadow-[0_0_30px_rgba(167,139,250,0.14)]">
          <p className="inline-flex rounded-full border border-violet-400/30 bg-violet-400/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-violet-300">
            {copy.cards.apex.badge}
          </p>
          <h3 className="mt-3 text-xl font-bold text-white">{copy.cards.apex.name}</h3>
          <p className="mt-3 text-3xl font-extrabold text-white transition-all duration-300">
            €{price.apex} <span className="text-sm font-medium text-[#A1A1AA]">{mode === "monthly" ? copy.perMonthSuffix : copy.perYearSuffix}</span>
          </p>
          <Button className="mt-4 w-full" disabled={working !== null} onClick={() => void checkout("apex")}>
