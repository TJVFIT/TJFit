...
export function CoachProfileView({ locale, slug }: { locale: string; slug: string }) {
  ...
  if (loading) return <div className="mx-auto max-w-4xl px-4 py-14 text-sm text-zinc-400">Loading coach profile...</div>;
  if (!data) return <div className="mx-auto max-w-4xl px-4 py-14 text-sm text-zinc-400">Coach not found.</div>;
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5 px-4 py-10">
      <section className="rounded-2xl border border-[#1E2028] bg-[#111215] p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">@{coach.username}</p>
      ...
      <section className="grid gap-3 rounded-2xl border border-[#1E2028] bg-[#111215] p-4 md:grid-cols-4">
        {[
          ...
          <article key={String(label)} className="rounded-lg border border-[#1E2028] bg-[#0E0F12] p-3">
      ...
      <section className="rounded-2xl border border-[#1E2028] bg-[#111215] p-5">
        ...
        <p className="mt-3 whitespace-pre-wrap text-sm text-zinc-300">{about || "No detailed profile yet."}</p>
      ...
      <section className="rounded-2xl border border-[#1E2028] bg-[#111215] p-5">
        ...
        <div className="mt-3 space-y-2">
          {(coach.certifications ?? []).map((cert) => (
            <p key={cert} className="inline-flex items-center gap-2 text-sm text-zinc-300">
              <Check className="h-4 w-4 text-green-400" /> {cert}
            </p>
          ))}
          {(coach.certifications ?? []).length === 0 ? <p className="text-sm text-zinc-500">No certifications listed.</p> : null}
        </div>
      </section>

      <section className="rounded-2xl border border-[#1E2028] bg-[#111215] p-5">
        <p className="text-xs uppercase tracking-[0.14em] text-zinc-500">Most popular program by this coach</p>
        ...
        {coach.featured_program_id ? (
          <Link href={`/${locale}/programs/${encodeURIComponent(coach.featured_program_id)}`} className="mt-3 inline-flex rounded-full bg-[#22D3EE] px-4 py-2 text-sm font-semibold text-[#09090B]">
            View Program
          </Link>
        ) : null}
      </section>
