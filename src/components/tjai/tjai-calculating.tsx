...
export function TJAICalculating({ copy, metrics, done = false }: Props) {
  ...
  return (
    <section className="relative min-h-[100svh] overflow-hidden bg-[#09090B] px-4 py-16 text-white">
      <div className="pointer-events-none absolute left-1/2 top-[-160px] h-[500px] w-[500px] -translate-x-1/2 rounded-full bg-[#22D3EE] opacity-10 blur-[60px]" />
      <div className="pointer-events-none absolute bottom-[-120px] right-[-80px] h-[400px] w-[400px] rounded-full bg-[#A78BFA] opacity-10 blur-[60px]" />

      <div className="mx-auto flex min-h-[75svh] w-full max-w-2xl flex-col items-center justify-center">
        <div className="rounded-2xl border border-[#1E2028] bg-[#111215]/70 px-6 py-10 text-center backdrop-blur">
          ...
          <p key={msgIdx} className="mt-3 text-sm text-[#A1A1AA] animate-[tjai-fade-in_300ms_ease]">
            {currentMessage}
          </p>

          <div className="mt-8 h-[2px] w-full overflow-hidden rounded-full bg-[#1E2028]">
            <div
          ...
          <div className="mt-6 grid gap-3">
            {stats.map((s, i) => (
              <div
                key={s}
                className={cn(
                  "rounded-xl border border-[#1E2028] bg-[#111215] px-6 py-4 text-sm text-[#D4D4D8] opacity-0",
