...
export function HeroTjaiBrainDeco({ reduce, active }: { reduce: boolean; active: boolean }) {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const ry = Math.sin(now * 0.00042) * 6.5;
      const rx = Math.cos(now * 0.00034) * 3.2;
      const rz = Math.sin(now * 0.00026) * 1.6;
    ...
    >
      ...
      >
        <div
          ...
          style={{
            background:
              "radial-gradient(circle at 42% 38%, rgba(34,211,238,0.35) 0%, rgba(14, 165, 233,0.12) 42%, transparent 68%)"
          }}
        />
        <div
          ref={wrapRef}
        ...
        >
          ...
          <img
            ...
            style={{
              filter:
                "drop-shadow(0 0 20px rgba(34,211,238,0.55)) drop-shadow(0 0 48px rgba(14, 165, 233,0.2))"
            }}
          />
        </div>
      </div>
