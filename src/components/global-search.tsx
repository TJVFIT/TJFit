...
export function GlobalSearch({ locale, collapsed, onExpand }: { locale: string; collapsed: boolean; onExpand?: () => void }) {
  ...
  return (
    <div className="relative mt-2 px-2">
      <div className="group/gsearch flex items-center gap-2 rounded-lg border border-divider bg-surface px-3 py-2 transition-[border-color,box-shadow] duration-200 focus-within:border-cyan-300/45 focus-within:shadow-[0_0_22px_rgba(34,211,238,0.16)]">
        <Search className="h-4 w-4 text-dim transition-colors duration-200 group-focus-within/gsearch:text-cyan-200" />
        <input
          value={query}
      ...
      {open ? (
        <div className="absolute left-2 right-2 top-[calc(100%+6px)] z-[120] max-h-[420px] overflow-y-auto rounded-xl border border-divider bg-surface p-3">
          ...
          {searchNormalize(query).length >= 2 ? (
            <Link href={`/${locale}/search?q=${encodeURIComponent(query)}`} className="tj-cta-sheen block rounded-md border border-divider px-2 py-2 text-center text-xs text-accent transition-[border-color,color,box-shadow] duration-200 hover:border-cyan-300/40 hover:text-cyan-50 hover:shadow-[0_0_14px_rgba(34,211,238,0.12)]">
              View all results
            </Link>
          ) : null}
        </div>
