...
function renderBlocks(blocks: FreeProductBlock[], dayLabels: { warmup: string; main: string; cooldown: string }) {
  return blocks.map((block, i) => {
    ...
    switch (block.type) {
      ...
      case "h3":
        return (
          <h3 key={key} className="mt-6 text-lg font-semibold text-bright">
            {block.text}
          </h3>
        );
      case "p":
        return (
          <p key={key} className="mt-3 text-sm leading-7 text-muted">
            {block.text}
          </p>
        );
      case "ul":
        return (
          <ul key={key} className="mt-3 list-inside list-disc space-y-2 text-sm leading-7 text-bright">
      ...
      case "day":
        return (
          <div key={key} className="mt-6 rounded-[20px] border border-white/10 bg-white/[0.04] p-5 sm:p-6">
            ...
            <p className="mt-3 text-xs uppercase tracking-[0.2em] text-faint">{dayLabels.warmup}</p>
            <ul className="mt-2 space-y-1 text-sm text-bright">
              {block.day.warmupLines.map((line, idx) => (
                <li key={`${line}-${idx}`}>{line}</li>
              ))}
            </ul>
            <p className="mt-5 text-xs uppercase tracking-[0.2em] text-faint">{dayLabels.main}</p>
            <ul className="mt-3 space-y-3">
              {block.day.exercises.map((ex, j) => (
                <li key={`${ex.line}-${j}`} className="text-sm text-bright">
                  <span className="font-medium text-white">{ex.line}</span>
                  {ex.note ? <span className="mt-1 block text-faint">{ex.note}</span> : null}
                </li>
              ))}
            </ul>
            <p className="mt-5 text-xs uppercase tracking-[0.2em] text-faint">{dayLabels.cooldown}</p>
            <ul className="mt-2 space-y-1 text-sm text-bright">
...
export function FreeProductUpgradeFooter({
  ...
  return (
    <div className="mx-auto max-w-[540px] border-t border-divider bg-surface px-8 py-12 text-center">
