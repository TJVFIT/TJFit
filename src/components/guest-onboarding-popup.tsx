...
const MARKETING_DEFER_SESSION_KEY = "tjfit_marketing_deferred_this_session";
...
export function GuestOnboardingPopup({ locale }: { locale: Locale }) {
  ...
  useEffect(() => {
    ...
    const marketingDeferred =
      sessionStorage.getItem(MARKETING_DEFER_SESSION_KEY) === "1";

    if (marketingFlag && !marketingDone && !marketingDeferred) {
      setStage("marketing");
      return;
    }
  ...
  const chooseCreateAccount = () => {
    ...
    sessionStorage.setItem(MARKETING_DEFER_SESSION_KEY, "1");
    setStage("hidden");
      ...
      >
        {stage === "entry" && (
          <>
            ...
            <h2 id="guest-onboarding-title" className="mt-3 text-2xl font-semibold text-white">{copy.entryTitle}</h2>
        ...
        {stage === "marketing" && (
          <>
            ...
            <h2 id="guest-onboarding-title" className="mt-3 text-2xl font-semibold text-white">{copy.marketingTitle}</h2>
            <p className="mt-3 text-sm text-zinc-400">
        ...
        {stage === "email" && (
          <>
            ...
            <h2 id="guest-onboarding-title" className="mt-3 text-2xl font-semibold text-white">{copy.enterEmail}</h2>
