...
export default function RecordsPage({ params }: { params: { locale: string } }) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8 px-4 py-16 sm:px-6 lg:px-8">
      ...
      <div>
        <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-dim">Records</p>
        <h1 className="mt-3 font-display text-3xl font-semibold text-white sm:text-4xl">Personal Records</h1>
        <p className="mt-3 max-w-2xl text-sm leading-7 text-muted">Your all-time bests auto-computed from your workout logs.</p>
      </div>
      ...
      {!loading && !error && prs.length === 0 && (
        <div className="flex flex-col items-center rounded-2xl border border-divider bg-surface py-20 text-center">
          ...
          <p className="mt-2 max-w-xs text-sm text-muted">Log workouts with sets, reps, and weight to see your personal bests here.</p>
          <a href={`/${locale}/progress`} className="mt-6 inline-flex items-center justify-center rounded-full border border-accent/30 bg-accent/10 px-6 py-2.5 text-sm font-semibold text-accent">Go to progress log →</a>
        </div>
      )}
          ...
          {filtered.map((pr) => (
            <div key={pr.exercise} className="rounded-2xl border border-divider bg-surface p-5 transition-[border-color,box-shadow] duration-200 hover:border-[rgba(34,211,238,0.2)]">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className="shrink-0 text-xs text-dim">{pr.total_sets} sets</span>
              </div>
              ...
              <p className="mt-3 text-xs text-faint">Total sets logged: {pr.total_sets}</p>
            </div>
          ))}
        </div>
      )}
