...
/**
 * Editorial obsidian + accent PDF theme shared by program and diet PDFs.
...
/** Editorial hairline + accent wordmark header. */
export function drawCoverHeader(pdf: jsPDF, subtitle: string) {
  setDraw(pdf, PDF_THEME.accent);
...
/** Footer — page number + accent serif mark. */
...
/** Accent gradient bar approximated via stacked rects (jsPDF has no gradients). */
export function drawAccentBar(pdf: jsPDF, x: number, y: number, w: number, h: number) {
  const steps = 18;
  for (let i = 0; i < steps; i++) {
    const t = i / (steps - 1);
    const r = Math.round(PDF_THEME.accentHi.r + (PDF_THEME.accentSoft.r - PDF_THEME.accentHi.r) * t);
    const g = Math.round(PDF_THEME.accentHi.g + (PDF_THEME.accentSoft.g - PDF_THEME.accentHi.g) * t);
    const b = Math.round(PDF_THEME.accentHi.b + (PDF_THEME.accentSoft.b - PDF_THEME.accentHi.b) * t);
