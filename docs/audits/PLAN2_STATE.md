...
| 15 | E | Form/error microcopy pass (role="alert") | done | sha-next | Checkout: stripped raw data.error / e.message leaks into UI, added role=alert/status + aria-live to status pill. TJAI quiz + support: added role=alert to validation/error <p>. Signup: 2 hardcoded EN strings → new chooseUsernameFirst / chooseGoalFirst keys in launch-copy.ts across all 5 locales. |
