...
## ⚖️ Before scaling newsletter sends: add unsubscribe (CAN-SPAM/GDPR)
The newsletter welcome email (`EmailTemplates.newsletterPlanWelcome`, sent from `api/newsletter/confirm`) carries marketing content but has **no unsubscribe link**, there's **no `List-Unsubscribe` header** in `src/lib/email.ts`, and there's **no `/api/newsletter/unsubscribe` endpoint** for email-only subscribers (the existing `/api/email/unsubscribe` is userId-based; newsletter subs may have no account). Commercial email legally needs a working opt-out. Not urgent pre-launch (no mass sends), but **required before scaling newsletter/marketing email**. Fix = an email-token unsubscribe endpoint that sets `newsletter_subscribers.unsubscribed_at`, plus a `footerUrl` on the template + a `List-Unsubscribe` header. Tell me to build it and I will (it needs a careful token design, so I didn't do it blind). *(Transactional emails — purchase/PDF/welcome — already include unsubscribe and are fine.)*
